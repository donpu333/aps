...
class BinanceAPIManager {
  ...
  _updateConnectionState(stateUpdate) {
    this.connectionState = { ...this.connectionState, ...stateUpdate, lastCheck: new Date().toISOString() };
...
function validateForm() {
  ...
  if (telegramCheckbox && telegramCheckbox.checked) {
    const savedChatId = localStorage.getItem('tg_chat_id') || document.getElementById('userChatId')?.value;
    if (!savedChatId) {
      showBotConnectionHint();
      isValid = false;
    }
      ...
      <div>
        ...
        <div id="editCurrentPriceContainer" class="current-price-container">
          ...
          <button type="button" onclick="applyCurrentPriceForEdit()" class="apply-price-btn" title="Применить текущую цену">
            <i class="fas fa-sync-alt"></i><span>Применить</span>
          </button>
        </div>
      </div>
      <div>
